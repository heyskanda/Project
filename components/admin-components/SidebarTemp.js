...
export default function SidebarTemp() {
    ...
    const showSidebar = () => setToggleCollapse(!toggleCollapse)
    ...
    return (
        <div ref={sidebarRef} className={toggleCollapse ? 'z-10 top-0 left-0 fixed h-screen w-20 bg-slate-700 p-4 flex flex-col justify-start transition-x-0 ease-in duration-200' : 'z-10 fixed h-screen w-56 bg-slate-700 p-4 flex flex-col justify-start transition-x-full ease-in duration-200'}>
            <div className='flex flex-row justify-between items-center'>
                <div className={toggleCollapse ? '-translate-x-32 ease-in-out duration-200' : 'translate-x-0 ease-in-out duration-200'}>
                    <svg width="120" height="35" viewBox="0 0 375 129" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <button className={toggleCollapse ? 'hover:bg-zinc-800 -translate-x-[7.6rem] ease-in-out duration-200 text-white p-1 rounded rotate-90' : 'translate-x-0 ease-in-out duration-200 hover:bg-zinc-800 text-white p-1 rounded'} onClick={showSidebar}>
                    <GoIcons.GoThreeBars fontSize={24} style={{fill:'rgb(241, 245, 249)'}} />
                </button>
            </div>
            <nav className='mt-8'>
                <ul>
                    {/* <li className='p-1 flex flex-row gap-2 items-center text-slate-100 rounded-md cursor-pointer hover:bg-zinc-800 ease-in duration-300 '>
                        <a className={toggleCollapse ? 'flex flex-row items-center' : 'flex flex-row items-center p-1'}>
                            <BiSearchAlt fontSize={24}/>
                            <span className={toggleCollapse ? 'hidden' : 'px-2'}>Search</span>
                        </a>
                    </li> */}
                    {SidebarData.map((item, index) => {
                        return(
                            <Link href={item.path}>
                                <li className='my-3 p-1 py-2 text-slate-100 rounded-md gap-2 cursor-pointer hover:bg-zinc-800 ease-in duration-300'>
                                        <a className={toggleCollapse ? 'flex flex-row items-center' : 'flex flex-row items-center p-1'}>
                                            {item.icon}
                                            <span className={toggleCollapse ? 'hidden' : 'w-full px-2 text-base'}>{item.title}</span>
                                        </a>
                                </li>
                            </Link>
                        )
                    ...
                    <li className='my-3 p-1 py-2 text-slate-100 rounded-md gap-2 cursor-pointer hover:bg-zinc-800 ease-in duration-300'>
                        <a  
                        onClick={logOut}
                        className={toggleCollapse ? 'flex flex-row items-center' : 'flex flex-row items-center p-1'}>
                            <MdIcons.MdLogout fontSize={24}/>
                            <span className={toggleCollapse ? 'hidden' : 'w-full px-2 text-base'}>Logout</span>
                        </a>
                    </li>
                </ul>
            </nav>
